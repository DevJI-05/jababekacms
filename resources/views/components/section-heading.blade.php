...
<div class="flex items-center justify-between gap-4">
    <h2 class="text-xl font-bold text-brand-text sm:text-2xl">{{ $title }}</h2>

    @if ($viewAllHref)
        <a href="{{ $viewAllHref }}" class="flex shrink-0 items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-accent hover:text-accent-dark">
            {{ $viewAllLabel }}
