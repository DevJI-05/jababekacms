...
@if ($layout === 'horizontal')
    <a href="{{ $href }}" class="group flex gap-4 rounded border border-slate-200 bg-white p-4 transition-shadow hover:shadow-md">
        ...
        <div class="flex min-w-0 flex-1 flex-col gap-1.5">
            <span class="inline-block w-fit rounded bg-surface-mint px-2 py-0.5 text-[11px] font-semibold text-accent">{{ $date }}</span>
            ...
            <span class="flex items-center gap-1 text-xs font-bold uppercase tracking-wide text-accent group-hover:text-accent-dark">
                {{ __('Read more') }}
...
@else
    <a href="{{ $href }}" class="group flex flex-col overflow-hidden rounded border border-slate-200 bg-white transition-shadow hover:shadow-lg">
